...
export default function LeetcodeHeatmap() {
  ...
  const submissionsByDate = submissions?.reduce((acc: Record<string, SubmissionData>, submission) => {
  ...
  const getClassForValue = (value: CalendarHeatmap.ReactCalendarHeatmapValue<string> | undefined) => {
  ...
  if (!isSignedIn) {
    return <div className="text-center p-4">Sign in to track your LeetCode progress</div>;
  }
      ...
      {tooltipData && tooltipData.count > 0 && (
        ...
        >
          ...
          <div className="text-sm mb-2">
            {tooltipData.count} problem{tooltipData.count !== 1 ? 's' : ''} solved
          </div>
          
          {tooltipData.problemTitles && tooltipData.problemTitles.length > 0 && (
